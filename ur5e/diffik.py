...
def main() -> None:
    ...
    # 【修复关键】：直接用 dof_ids 去底层数组取 qposadr，保证它是干净的 (6,) 形状
    qpos_ids = model.jnt_qposadr[dof_ids]

    # 单独定义驱动器的名称列表，与 xml 文件中的 name 属性对应
    actuator_names = [
        "shoulder_pan",
        "shoulder_lift",
        "elbow",
    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            data.mocap_pos[mocap_id, 0:2] = circle(data.time, 0, 0.5, 0.0, 0.5)
            ...
            # ---------------- 限位与控制 ----------------
            # 1. 提取我们实际控制的 6 个机械臂关节的目标角度 (形状为 6,)
            q_target = q[qpos_ids]

            # 2. 获取这 6 个关节的限位范围
            jnt_ranges = model.jnt_range[dof_ids]

            # 3. 仅对这 6 个关节的目标值进行限位裁剪
